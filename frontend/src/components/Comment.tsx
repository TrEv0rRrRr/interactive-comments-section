...
const Comment = () => {
  ...
  const startReplying = (parentId: number, content: string) => {
    setReplyingContent(content);
    setAnswerId(parentId);
  };
  ...
  const handleReply = async (
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const reply = await createComment(replyContent, options);
    } catch (error) {
      console.error(error);
      throw error;
    }
  ...
  return (
    <div className="flex flex-col p-1 gap-5">
      {comments.map(
        ({ content, score, id, userId, createdAt, replyingTo, parentId }) => {
                ...
                {isEditing && isCurrentUser ? (
                  <div>
                    <button
                      ...
                      onClick={() =>
                        handleEdit(id, score, replyingTo, parentId)
                      }
                    >
                      UPDATE
                    </button>
